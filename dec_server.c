...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (pid == 0) {
      ...
      char key[1024];
      int bytes_received = recv(client_fd, ciphertext, sizeof(ciphertext), 0);
      if (bytes_received == -1) {
        perror("recv");
        exit(1);
      }
      ...
      int bytes_sent;
      bytes_sent = send(client_fd, plaintext, sizeof(plaintext), 0);
      if (bytes_sent == -1) {
        char *error_message = strerror(errno);
        fprintf(stderr, "Error sending plaintext to client: %s\n", error_message);
        exit(1);
      }
      close(client_fd);
      exit(0);
